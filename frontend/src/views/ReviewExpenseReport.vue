...
export default {
    ...
    methods: {
        review(item, status) {
            this.axios.put("/expense-reports/" + item.id, status,{headers: {'Content-Type': 'application/json', 'Authorization': `Bearer ` + localStorage.jws}})
                .then(() => {
                    item.status = status;
                })
        },
    ...
    mounted() {
        this.axios.get("/admin/expense-reports", {headers: {'Authorization': `Bearer ` + localStorage.jws}})
            .then(r => {
